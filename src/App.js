...
const CONTRACT_ADDRESS = "0x3174BAe63652494D62F6f4709d2fb659C2F160cb";

const App = () => {
  // ユーザーのウォレットアドレスを格納するために使用する状態変数を定義します。
  const [currentAccount, setCurrentAccount] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [mntCount, setMntCount] = useState(0);
  ...
  const setupEventListener = async () => {
    try {
      ...
      if (ethereum) {
        ...
        let getCount = await connectedContract.getCount()
        setMntCount(getCount.toNumber())
    ...
    <div className="App">
      <div className="container">
        <div className="header-container">
          ...
          <p className="sub-text">これまでに作成された {mntCount}/{TOTAL_MINT_COUNT} NFT</p>
          {/*条件付きレンダリング。
          // すでにウォレット接続されている場合は、
          // Mint NFT を表示する。*/}
          {
              isLoading ? (<p className="sub-text">Loading......</p>) : ("")
          }
          {
              renderContent()
          }
